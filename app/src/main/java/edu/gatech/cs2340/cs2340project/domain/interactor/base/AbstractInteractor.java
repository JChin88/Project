 ...
 * why this class was created. Field methods are declared volatile as we might use these methods
 * from different threads (mainly from UI).
 * <p/>
 * For example, when an activity is getting destroyed then we should probably cancel an interactor
 * but the request will come from the UI thread unless the request was specifically assigned
...
public abstract class AbstractInteractor implements Interactor {
     ...
     * DIRECTLY but, instead, a developer should call the execute() method of an interactor to make
     * sure the operation is done on a background thread.
     * <p/>
     * This method should only be called directly while doing unit/integration tests. That is the
     * only reason it is declared public as to help with easier testing.
     */
